...
- `CACHE_NEW_MARKETS` - Set to `true` to cache new markets.
  - This option should not be used with public RPC.
- `TRANSACTION_EXECUTOR` - Set to `turbo` to use turbo infrastructure for executing transactions, or set it to jito to use JSON-RPC jito executer
  - For more details, check out [turbo](#turbo-transactions-rc) section
- `CUSTOM_FEE` - If using turbo or jito executors this value will be used for transaction fees instead of `COMPUTE_UNIT_LIMIT` and `COMPUTE_UNIT_LIMIT`
...
## Turbo transactions (RC)

In case you experience a lot of failed transactions or transaction performance is too slow, you can try using `turbo` for executing transactions.
